                       ...
                       __global float *debug)
{
  ...
  __local vertex local_vertices[8][8];

  local_vertices[get_local_id(1)][get_local_id(0)] = v[get_global_id(1)*image_size_x + get_global_id(0)];
...
//    local_vertices[final_local] = v[final_global];
  ...
  //   for (y=(int)get_local_id(1); y<=(int)get_local_id(1)+SIZE*2; y++){
   for (x=(int)get_local_id(0)-SIZE; x<=(int)get_local_id(0)+SIZE; x++){
    for (y=(int)get_local_id(1)-SIZE; y<=(int)get_local_id(1)+SIZE; y++){    /*if(y < 0) {
       continue;
       }
       if(x < 0){
       continue;
       ...
       if (x<0 || x>=8 || y<0 || y>=8)
       {
      		index = get_global_id(1) * image_size_x + get_global_id(0);//----
      		if (index < 0 || index >= image_size_x)
       ...
       {
      		index = y * 8 + x;
      		position_h = (float3)(local_vertices[x][y].x, local_vertices[x][y].y, local_vertices[x][y].z);
      		normal = normalize((float3)(local_vertices[x][y].nx, local_vertices[x][y].ny, local_vertices[x][y].nz));
	   }
//      if (x==3 && y==3)
//      {
