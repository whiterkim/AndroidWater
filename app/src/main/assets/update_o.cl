                       ...
                       __global float *debug)
{
  ...
  //int start_global = get_global_id(0) - SIZE * image_size_x -SIZE;  
  int start_local = 0;
  ...
  for(k = 0; k < LOOP_COUNT; ++k){
	...
	if (current_column + local_offset % LOCAL_SIZE > LOCAL_SIZE)
    {
  ...
  /*if (i==3 && j==3)
  {
    ...
    for(y = (int)i - SIZE; y <= (int)i + SIZE; ++y){
  		debug[i]=v[y * image_size_x + x].x;
  		i+=2;
  	}
  	}
  	i=1;
  	for (x=get_local_id(0); x<=get_local_id(0)+SIZE*2; x++){
  	for (y=get_local_id(1); y<=get_local_id(1)+SIZE*2; y++){
  		debug[i]=local_vertices[y * LOCAL_MEM_SIZE + x].x;
  		i+=2;
  	}
  	}
  }*/
